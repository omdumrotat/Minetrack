...
class Database {
  constructor (app) {
    this._app = app
    const databasePath = path.join(__dirname, '..', 'data', 'database.sql')
    this._sql = new sqlite.Database(databasePath)
  }
  ...
  loadRecords (callback) {
    ...
    // If no servers are registered, continue startup to avoid exiting
